...
class TinyViT(nn.Module):
    def __init__(self, dim=96, depth=8, heads=8, patch=4):
        super().__init__()
        self.patch, self.depth, self.g = patch, depth, 28 // patch
        self.proj = nn.Linear(patch * patch, dim)
        self.pos = nn.Parameter(torch.randn(1, self.g * self.g, dim) * .02)
        self.block = Block(dim, heads)
        self.depth_emb = nn.Parameter(torch.zeros(depth, 1, 1, dim))
        self.norm = nn.LayerNorm(dim)
        self.head = nn.Linear(dim, patch * patch)
        nn.init.zeros_(self.head.weight); nn.init.zeros_(self.head.bias)
    def forward(self, x):
        k, g, B = self.patch, self.g, x.shape[0]
