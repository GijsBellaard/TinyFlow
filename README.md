...
- `--skip-train`: Skip training and only sample from the model (default: False). The model options must match the ones used to train the checkpoint.
...
- The model is a vision transformer and works with $4\times4$ patches.
...
- A exponential moving average of the model weights is maintained during training and used for sampling.